...
from Ano import ASSID, BOT_ID, MUSIC_BOT_NAME, OWNER_ID, SUDOERS, app
from Ano import boottime as bot_start_time
from Ano import db
from Ano.Core.PyTgCalls import Ano
from Ano.Database import (add_nonadmin_chat, add_served_chat,
...
from Ano.Decorators.admins import ActualAdminCB
from Ano.Decorators.permission import PermissionCheck
from Ano.Inline import (custommarkup, dashmarkup, setting_markup,
                          start_pannel, usermarkup, volmarkup)
from Ano.Utilities.ping import get_readable_time
...
@app.on_callback_query(filters.regex("goback"))
...
async def start_markup_check(_, CallbackQuery):
    ...
    if command == "LV":
        ...
        volume = 25
        try:
            await Ano.pytgcalls.change_volume_call(c_id, volume)
    ...
    if command == "DIT":
        ...
        await CallbackQuery.answer(
            f"Ano Disk Usage: {diske}%", show_alert=True
        )
