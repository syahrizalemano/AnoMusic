...
from Ano import BOT_USERNAME, MUSIC_BOT_NAME, app, db_mem
from Ano.Core.PyTgCalls import Queues, Ano
from Ano.Core.PyTgCalls.Converter import convert
from Ano.Core.PyTgCalls.Downloader import download
from Ano.Database import (_get_playlists, delete_playlist, get_playlist,
                            get_playlist_names, is_active_chat, save_playlist)
from Ano.Database.queue import (add_active_chat, is_active_chat,
                                  is_music_playing, music_off, music_on,
                                  remove_active_chat)
from Ano.Decorators.admins import AdminRightsCheckCB
from Ano.Decorators.checker import checkerCB
from Ano.Inline import (audio_markup, audio_markup2, download_markup,
                          fetch_playlist, paste_queue_markup, primary_markup)
from Ano.Utilities.changers import time_to_seconds
from Ano.Utilities.chat import specialfont_to_normal
from Ano.Utilities.paste import isPreviewUp, paste_queue
from Ano.Utilities.theme import check_theme
from Ano.Utilities.thumbnails import gen_thumb
from Ano.Utilities.timer import start_timer
from Ano.Utilities.youtube import get_yt_info_id
...
async def admin_risghts(_, CallbackQuery):
    ...
    if command == "pausecb":
        ...
        await Ano.pytgcalls.pause_stream(chat_id)
    ...
    if command == "resumecb":
        ...
        await Ano.pytgcalls.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
    ...
    if command == "stopcb":
        ...
        await Ano.pytgcalls.leave_group_call(chat_id)
        await CallbackQuery.message.reply_text(
    ...
    if command == "skipcb":
        ...
        if Queues.is_empty(chat_id):
            ...
            if str(finxx) != "raw":
                ...
                await Ano.pytgcalls.change_stream(
                    chat_id,
                    InputStream(
                        InputAudioStream(
                            raw_path,
...
async def play_playlist(_, CallbackQuery):
    ...
    if not _playlist:
        ...
        for_t = 0
        for_p = 0
        for AnoBots in _playlist:
            _note = await get_playlist(user_id, AnoBots, type)
            title = _note["title"]
            ...
            if await is_active_chat(chat_id):
                ...
                raw_path = await convert(downloaded_file)
                try:
                    await Ano.pytgcalls.join_group_call(
                        chat_id,
                        InputStream(
                            InputAudioStream(
                                raw_path,
...
async def check_playlist(_, CallbackQuery):
    ...
    if not _playlist:
        ...
        for AnoBots in _playlist:
            j += 1
            _note = await get_playlist(user_id, AnoBots, genre)
            title = _note["title"]
            duration = _note["duration"]
            msg += f"{j}- {title[:60]}\n"
...
async def del_playlist(_, CallbackQuery):
    ...
    if not _playlist:
        ...
        await CallbackQuery.answer()
        for AnoBots in _playlist:
            await delete_playlist(user_id, AnoBots, genre)
    await CallbackQuery.message.reply_text(
