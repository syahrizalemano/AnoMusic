...
from Ano import (ASSID, ASSMENTION, ASSNAME, ASSUSERNAME, BOT_ID, BOT_NAME,
                   BOT_USERNAME, SUDOERS, app, db, pymongodb, userbot)
from Ano.Core.Logger.Log import (startup_delete_last, startup_edit_last,
                                   startup_send_new)
from Ano.Core.PyTgCalls.Ano import run
from Ano.Database import get_active_chats, get_sudoers, remove_active_chat
from Ano.Inline import private_panel
from Ano.Plugins import ALL_MODULES
from Ano.Utilities.inline import paginate_modules
...
async def initiate_bot():
    with console.status(
        "[magenta] Booting up The Ano Music...",
    ) as status:
    ...
    console.print(
        "[bold green]Congrats!! Ano Music has started successfully!🥳\n"
    )
    try:
    ...
    try:
        ...
        return
    try:
        await userbot.join_chat("AnoBots")
    except:
        pass
...
@app.on_callback_query(filters.regex("AnoBots"))
async def AnoBots(_, CallbackQuery):
